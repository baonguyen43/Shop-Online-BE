...
router.get('/call-stored-procedure', async (req: Request, res: Response, next: any) => {
  try {
    const results = await repository.manager.connection.query('EXECUTE [dbo].[usp_Products_GetByDiscount] @0', [5.25]);
    res.json(toCamelCase(results));
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Internal server error' });
...
// HIển thị các khach hàng  ko bán dc trong khoảng ngày 
router.get('/get-customer-buy-onday', async (req: Request, res: Response, next: any) => {
  try {
    const fromDate = req.query.date;
    const toDate = req.query.date
    const results = await repository.manager.connection.query('EXECUTE [dbo].[usp_Customer_BuyDay] @FromDate = @0, @ToDate = @1', [fromDate, toDate ]);
    res.json(toCamelCase(results));
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Internal server error' });
